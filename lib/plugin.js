...
import { ACCEPTED, BAD_REQUEST, GATEWAY_TIMEOUT, HttpError, NotFoundError } from 'http-errors-enhanced'
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (metrics) {
        ...
        metrics.httpRequestDuration.observe({ topic, method, status_code: statusCode }, duration)
      }
...
function createResponseProcessor (logger, pendingRequests) {
  return async function processResponseMessage (message) {
    ...
    pendingRequest.reply.status(statusCode).header('content-type', contentType).send(message.value)
  }
}
        ...
        messages: [
          {
            ...
            headers: {
              ...
              ...Object.fromEntries(
                Object.entries(request.headers).filter(([key]) => key !== 'content-type' && key !== keyHeader)
              )
            }
          }
        ]
      ...
      server.route({
        ...
        async handler (request, reply) {
          ...
          const timeoutHandle = setTimeout(() => {
            pendingRequests.delete(correlationId)
            if (!reply.sent) {
              const error = new HttpError(GATEWAY_TIMEOUT, 'Request timeout')
              reply.status(error.status).send({ code: error.code, ...error.serialize() })
            }
          }, timeout)
          ...
          // Prepare additional headers for path params and query string
          const additionalHeaders = {}

          // Add path parameters if any
          if (request.params && Object.keys(request.params).length > 0) {
            additionalHeaders[pathParamsHeader] = JSON.stringify(request.params)
          }

          // Add query string parameters if any
          if (request.query && Object.keys(request.query).length > 0) {
            additionalHeaders[queryStringHeader] = JSON.stringify(request.query)
          }
