...
export async function processMessage (logger, dlqProducer, mappings, message) {
  ...
  const { url, dlq, method, retryDelay, retries, includeAttemptInRequests } = mappings[topic]
...
export async function plugin (server, opts) {
  ...
  server.route({
    ...
    async handler (request, reply) {
      ...
      let value
      if (request.headers[contentTypeHeader]?.startsWith('application/json')) {
        ...
        throw new UnsupportedMediaTypeError('Unsupported content type')
      }
